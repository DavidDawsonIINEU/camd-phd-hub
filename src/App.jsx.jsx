...
const deadlines = [
  { title: "Annual Review", date: "April 15", recurring: true, category: "Program", detail: "Submit your self-reflection and portfolio each spring. Tracks your progress and development. Contact David Dawson II with questions." },
  { title: "Professional Development Funds", date: "May 15", recurring: true, category: "Funding", detail: "$1,000 annually (fiscal year July 1–June 30) for conferences, memberships, workshops, and research support. Submitted via Concur. Unspent funds do not roll over." },
  { title: "Flex Fellowship Application", date: "2 months before semester", recurring: false, category: "Funding", detail: "Submit at least 2 months before the semester start. Summer requests most likely to be approved. Available only after completing coursework and qualifying exam.", formLabel: "Flex Fellowship Form", formUrl: FORMS.flexFellowship },
  { title: "Computer Request (New Students)", date: "August 1", recurring: false, category: "Equipment", detail: "New students must submit the computer request form by August 1 to receive their university-issued computer in their first semester." },
        ...
        <div className="slide-in" style={{ padding: "0 0 100px" }}>
          ...
          <div style={{ padding: "0 20px" }}>
            ...
            <div style={{ background: COLORS.surface, borderRadius: 14, border: `1px solid ${COLORS.border}`, marginBottom: 20, overflow: "hidden" }}>
              <MetaRow icon="◷" label="Time" value={selectedEvent.time + (selectedEvent.endTime ? ` – ${selectedEvent.endTime}` : "")} />
              {selectedEvent.location && <MetaRow icon="◎" label="Location" value={selectedEvent.location} border />}
              {selectedEvent.url && <a href={selectedEvent.url} target="_blank" rel="noreferrer"><MetaRow icon="↗" label="Event page" value="Open website" border accent /></a>}
            </div>
...
function MetaRow({ icon, label, value, border, accent }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 18px", borderTop: border ? `1px solid ${COLORS.border}` : "none" }}>
      <div style={{ fontSize: 15, color: accent ? COLORS.accent : COLORS.muted, width: 20, textAlign: "center" }}>{icon}</div>
      <div style={{ flex: 1 }}>
